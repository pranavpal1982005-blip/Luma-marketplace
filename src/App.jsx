      ...
      {authOpen && (
        <div className="modal-backdrop" onClick={() => setAuthOpen(false)} role="presentation">
          <form className="auth-modal" onSubmit={submitAuth} onClick={(event) => event.stopPropagation()}>
            ...
            <button className="switch-auth" type="button" onimport { useEffect, useMemo, useState } from "react";
import axios from "axios";
import "./App.css";
...
export default App;
Click={() => { setAuthMode(authMode === "login" ? "register" : "login"); setAuthMessage(""); }}>{authMode === "login" ? "New here? Create an account" : "Already have an account? Sign in"}</button>
          </form>
        </div>
      )}
      {checkoutOpen && (
